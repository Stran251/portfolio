...
-- which SB was the highest scorring
SELECT 
      SB
    , winner
    , winner_pts
    , loser
    , loser_pts
    , max(winner_pts + loser_pts) as total_pts
FROM 
    superbowls;

--which SB was the lowest scoring
SELECT 
      SB
    , winner
    , winner_pts
    , loser
    , loser_pts
    , min(winner_pts + loser_pts) as total_pts
FROM 
    superbowls;

--which team has won the most SB
select
      winner
    , count(*)
from
...
--which players have the most SB MVPs
select
      mvp
    , count(*)
from
...
select
    ...
    , case
        ...
        when (winner_pts - loser_pts) > 20 then "blowout game"
        else "normal game"
    end as type_of_game
from
    superbowls
...
--which state has hosted the most SB
select
      state
...
--which stadium has hosted the most SB
select
      stadium
    , count(*) as times_hosted
from
